...
export class BCalendarComponent extends BBusinessComponent {
  ...
  getSnapshot() {
    return {
      state: this.state,
      calendarInfo: BCalendarComponent.calendarInfo
    };
  }
  setSnapshot(snapshot) {
    let { state, calendarInfo } = snapshot;
    this.setState({ ...state });
    BCalendarComponent.calendarInfo = calendarInfo;
  }
  getFormat() {
    return Format;
  }
  ...
  proxyDidRespond(proxyResponse) {
    ...
    switch (key) {
      case 'getCalendarInfo': {
        if (!this.unMounted) {
          BCalendarComponent.calendarInfo = response.value;
  ...
  componentDidMount() {
    // this.setState({ calendarInfo:this.getCalendarInfo() });
    super.componentDidMount();
    
    if (BCalendarComponent.calendarInfo === undefined || !(BCalendarComponent.calendarInfo.length > 0)) {
      this.getCalendarInfo();
    }   
  }
...
BCalendarComponent.Format = Format;
export default BCalendarComponent;
