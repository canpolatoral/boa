...
import BOAOnePropertiesPanel from './BOAOnePropertiesPanel';
...
function getPropertyPlaceholders(node) {
  var selector = 'input[name], textarea[name], [data-value], [contenteditable], .BOAOneComponent';
 ...
 */
function PropertiesPanel(config, eventBus, modeling, propertiesProvider, commandStack, canvas, context) {
...
PropertiesPanel.prototype._init = function (config) {
  ...
  eventBus.on(['propertiesPanel.changed', 'propertiesPanel.resized'], function () {
    ...
    if (!scroller) {
      ...
      scroller.on('scroll', function (newActiveNode) {
  ...
  if (!this._mainPropertiesPanel) {
    ReactDOM.render(
      <BAppProvider theme={this._context.theme}>
        <BOAOnePropertiesPanel ref={r => (this._mainPropertiesPanel = r)} context={this._context} />
      </BAppProvider>,
      this._container
    );
  }
...
// panel prop state component dışına aktrılmak için yazıldı
PropertiesPanel.prototype.getPanelProperty = function () {
  return this._mainPropertiesPanel.state.componentprop;
};
...
PropertiesPanel.prototype.update = function (element) {
  ...
  element.context = this.getContext();

  var newTabs = this._propertiesProvider.getTabs(element);

  if (this._mainPropertiesPanel) {
    this._mainPropertiesPanel.propertiesProvider = this._propertiesProvider;
    this._mainPropertiesPanel.eventBus = this._eventBus;
    this._mainPropertiesPanel.modeling = this._modeling;
    this._mainPropertiesPanel.create(element);
    return;
  }
...
PropertiesPanel.prototype.getContext = function () {
...
PropertiesPanel.prototype._createPanel = function (element, tabs) {
  ...
  forEach(tabs, function (tab) {
    ...
    var tabNode = domify('<div class="bpp-properties-tab" data-tab="' + tab.id + '"></div>'),
      tabLinkNode = domify(
        '<li class="bpp-properties-tab-link">' + '<a href data-tab-target="' + tab.id + '">' + tab.label + '</a>' + '</li>'
      );
    ...
    forEach(groups, function (group) {
      ...
      var groupNode = domify(
        '<div class="bpp-properties-group" data-group="' +
        group.id +
        '">' +
      ...
      forEach(group.entries, function (entry) {
        ...
        if (React.isValidElement(html)) {
          ReactDOM.render(html, entryNode);
        } else {
