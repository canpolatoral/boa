...
PopupMenu.prototype.create = function (id, element) {
  ...
  var current = this._current = {
    provider: this._providers['bpmn-replaceReplaceMenu'],
    className: 'bpmn-replaceReplaceMenu',
    element: element
  };
  var newEntries = [], newHeaderEntries = [];
  forEach(id, function (newProvider) {
    if (!newProvider) {
      throw new Error('Provider is not registered: ' + newProvider);
    }
    ...
    if (newProvider.getHeaderEntries) {
      forEach(newProvider.getHeaderEntries(element), function (itemHeader) {
        newHeaderEntries[newHeaderEntries.length] = itemHeader;
      });
    }
    forEach(newProvider.getEntries(element), function (item) {
      newEntries[newEntries.length] = item;
    });
  });
  current.headerEntries = newHeaderEntries;
...
PopupMenu.prototype._createEntry = function (entry) {
  ...
  if (entry.imageUrl) {
    entryContainer.appendChild(domify('<img src="' + entry.imageUrl + '", style="' + entry.imageStyle + '" />'));
  }
  if (entry.label) {
  ...
  if (entry.customHtml) {
    entryContainer.appendChild(domify(entry.customHtml));
  }
