...
@BComponentComposer
export class BResizable extends Component {
  static propTypes = {
  ...
  componentWillReceiveProps({ width, height, disabled }) {
    if (width !== this.props.width) this.setState({ width });
    if (height !== this.props.height) this.setState({ height });
    if (disabled !== this.props.disabled) this.setState({ disabled });
  }
  ...
  onMouseMove({ clientX, clientY }) {
    ...
    this.setState({
      width: width !== 'auto' ? (width !== '100%' ? newWidth : '100%') : 'auto',
      height: height !== 'auto' ? (height !== '100%' ? newHeight : '100%') : 'auto',
    });
