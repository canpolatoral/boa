...
@BComponentComposer
@withStyles(styles)
export class BIconMenu extends BComponent {
  static defaultProps = {
        ...
        this.props.items ?
          (this.props.items.map((item) => {
            const rightIcon = item.items && item.items.length ?
              <RightArrow /> :
              item.rightIcon && (item.rightIcon.fontIcon || item.rightIcon.svgIcon) ? BIcon.getIcon(item.rightIcon) : null;
            const leftIcon = item.leftIcon && (item.leftIcon.fontIcon || item.leftIcon.svgIcon) ? BIcon.getIcon(item.leftIcon) : null;
