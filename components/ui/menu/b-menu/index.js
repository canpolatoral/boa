...
@BComponentComposer
@withStyles(styles)
export class BMenu extends BComponent {
    ...
    if (this.props.isMenuItemList && this.props.items) {
      ...
      menuItems = this.props.items.map((item) => {
        ...
        if (!item.props.primaryText) {
          return (<Divider style={{ marginBottom: '12px', marginTop: '12px' }} key={Math.random()} />);
        }
      ...
      menuItems = this.props.items.map((item) => {
        ...
        ) : <Divider style={{ marginBottom: '12px', marginTop: '12px' }} key={Math.random()} />;
      });
    }
