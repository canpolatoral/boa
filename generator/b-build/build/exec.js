...
function execWrapper(command, options) {
    ...
    return proc.progress( function(stream){
        stream.stdout.on('data', data => output(data, 'stdout'));
        stream.stderr.on('data', data => output(data, 'stderr'));
    })
    .then(result => {
...
export default { exec: execWrapper, safeExec, setExecOptions}