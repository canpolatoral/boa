...
const flatten = (nodes = [], options = {}) => {
      ...
      while (p) {
        ...
        if (p.state.total < 0) {
          if (options.throwOnError) {
            throw new Error(
              'The node might have been corrupted: id=' +
                JSON.stringify(p.id) +
                ', state=' +
            ...
            console &&
              console.log(
                'Error: The node might have been corrupted: id=%s, parent=%s, children=%s, state=%s',
                JSON.stringify(p.id),
                p.parent,
                p.children,
                JSON.stringify(p.state),
  ...
  while (stack.length > 0) {
    ...
    while (index < current.children.length) {
      ...
      if (open) {
        if (options.openNodes.indexOf(node) < 0) {
          options.openNodes.push(node);
        }
      }
