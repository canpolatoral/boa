...
class InfiniteTree extends events.EventEmitter {
  ...
  create() {
    if (this.options.el) {
      ...
      if (this.options.layout === 'table') {
        ...
        tableElement.className = classNames('infinite-tree', 'infinite-tree-table');
      ...
      this.scrollElement.className = classNames('infinite-tree', 'infinite-tree-scroll');
      this.contentElement.className = classNames('infinite-tree', 'infinite-tree-content');
  ...
  checkNode(node, checked) {
    ...
    const isRadio = !this.options.isMultiSelect;
    if (!this.options.canCheckChildsByParent) {
      isSingleSelection = true;
    } else if (this.options.isLeafCheckable) {
      isSingleSelection = true;
    } else if (isRadio) {
      isSingleSelection = true;
    }
    ...
    if (!isSingleSelection) {
      isSingleSelection = this.options.isSingleCheckable;
    }

    if (isRadio) {
      if (!this.options.isSingleCheckable) {
        if (node.parent && node.parent.children) {
          node.parent.children.forEach(element => {
            element.isSelected = false;
            element.state.checked = false;
            this.setCheckedNodes(element);
          });
        }
      } else {
        if (this.lastCheckedNode) {
          this.lastCheckedNode.isSelected = false;
          this.lastCheckedNode.state.checked = false;
          this.setCheckedNodes(this.lastCheckedNode);
        }
    ...
    if (checked === true) {
      ...
      node.state.indeterminate = isSingleSelection ? false : !!node.state.indeterminate;
      node.state.checked = (node.state.checked && node.state.indeterminate) || !node.state.checked;
      node.state.indeterminate = false;
    }
    node.isSelected = node.state.checked;
    this.setCheckedNodes(node);
    ...
    const updateChildNodes = parentNode => {
      ...
      while (childNode) {
        ...
        childNode.isSelected = childNode.state.checked;
        this.setCheckedNodes(childNode);
    ...
    const updateParentNodes = childNode => {
      ...
      while (parentNode && parentNode.state.depth >= 0) {
        ...
        this.setCheckedNodes(parentNode);
    ...
    this.lastCheckedNode = checked && node;
    return true;
  }
  ...
  closeNode(node, options) {
    const { async = false, asyncCallback = noop, silent = false } = { ...options };
  ...
  filter(predicate, options) {
    ...
    const traverse = (node, filterNode = false) => {
      ...
      node.isExpanded = node.state.filtered;
    ...
    for (let i = 0; i < this.nodes.length; ++i) {
      ...
      // performans sorununa neden oluyor
      // if (node.isExpanded) {
      //   this.openNode(node);
      // }
    }
  ...
  getNodeById(id) {
    ...
    if (!node) {
      ...
      node = this.nodes.filter(node => node.id === id)[0]; // eslint-disable-line
      if (!node) {
        return null;
      }
      this.nodeTable.set(node.id, node);
  ...
  findCheckedAndOpenNodes(nodes, openNodes, checkedNodes, selectedNodes) {
    (nodes || []).forEach(node => {
      if (node.state.open) openNodes.push(node);
      ...
      if (node.state.selected) selectedNodes.push(node);
  ...
  loadData(data = []) {
    this.nodes = flatten(data, { openAllNodes: this.options.expandAll });
    ...
    if (this.state.checkedNodes.length > 0) {
      this.state.checkedNodes.forEach(node => {
  ...
  openNode(node, options) {
    ...
    const fn = () => {
      ...
      if (nodeIndex >= 0) {
        ...
        rows.length = nodes.length;
        for (let i = 0; i < nodes.length; ++i) {
          const node = nodes[i]; // eslint-disable-line
          rows[i] = this.options.rowRenderer(node, this.options);
        }
  ...
  removeChildNodes(parentNode, options) {
    ...
    parentNode.isExpanded = parentNode.isExpanded && parentNode.children.length > 0;
  ...
  selectNode(node = null, options) {
    ...
    // Select this node
    if (this.state.selectedNode !== node && nodeIndex > -1) {
  ...
  setCheckedNodes(node) {
    const index = this.state.checkedNodes.indexOf(node);
    if (index < 0) {
      if (node.isSelected) {
        this.state.checkedNodes = [node].concat(this.state.checkedNodes);
      }
    } else if (!node.isSelected) {
      if (this.state.checkedNodes.length > 0) {
        this.state.checkedNodes.splice(index, 1);
      }
    }
  }

  getCheckedNodes() {
    return this.state.checkedNodes || [];
  }
