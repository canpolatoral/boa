...
export class BDialog extends BComponent {
  ...
  getResourceInfo(resourceCode) {
    BComponent.FormManager.getResourceInfo(0, resourceCode, response => {
      if (response && response.success) {
        !this.props.title && this.setState({ title: response.value.name }); // this.prepareTitleWithCloseButton(response.value.name);
        this.setState({ resourceInfo: response.value });
      } else if (response && response.response && response.response.results) {
        BDialogHelper.showError(
          ...
          this.getMessage('BusinessComponents', 'ErrorText'),
          () => {
        ...
        BDialogHelper.show(
          this.props.context,
          this.getMessage('BusinessComponents', 'ErrorDuringTransaction'),
          BComponent.DialogType.ERROR,
          BComponent.DialogResponseStyle.OK,
          this.getMessage('BusinessComponents', 'ErrorText'),
          () => {
  ...
  prepareDialog() {
    ...
    if (!children) {
      ...
      if (this.state.open && this.props.resourceCode && !this.state.resourceInfo) {
        dialogContent = <BLoading context={this.props.context} />;
        let newResourceCode;
        if (this.props.resourceCode == 'getFromDefaultProps') {
          if (content && content.props && content.props.resourceCode) newResourceCode = content.props.resourceCode;
          else newResourceCode = this.props.resourceCode;
        } else {
          newResourceCode = this.props.resourceCode;
        }
        this.getResourceInfo(newResourceCode);
      } else if (this.state.resourceInfo) {
        dialogContent = React.cloneElement(dialogContent, { resourceInfo: this.state.resourceInfo });
      }
      // else {
      //   children = dialog.dialogContent; // this.props.content ? this.props.content :  dialog.children;
      // }
    } else {
...
export class BDialogHelper {
    ...
    const createAction = (context, text, type, focus) => {
      return (
        <BButton
          ...
          type="flat"
          text={BComponent.Messaging.getMessage('BOA', text)}
  ...
  static showWithResourceCode(
    ...
    if (!resourceCode) {
      resourceCode = 'getFromDefaultProps';
    }
    this.show(context, content, dialogType, dialogResponseStyle, title, onClose, style, resourceCode, onClosing);
  }
