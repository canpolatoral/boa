...
export class InputSearch extends BComponent {
  ...
  filter(searchText) {
    ...
    if (searchText.length > 0) {
      ...
      return this.props.dataSource.filter((item) => BComponent.Localization.stringLowerCase(item.label).indexOf(searchText) !== -1);
    } else {
  ...
  onChange(event, newValue) {
    if (newValue != undefined) {
      this.state.dataSourceFiltered = this.filter(BComponent.Localization.stringLowerCase(newValue));
    }
